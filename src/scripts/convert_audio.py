...
Usage:
    # Single file
    python convert_audio.py recording.wav
    python convert_audio.py recording.wav --format mp3
    python convert_audio.py recording.wav --format ogg mp3

    # Directory (converts all WAV files in the folder)
    python convert_audio.py recordings/

    # Glob pattern
    python convert_audio.py "recordings/2026-05-*.wav"

    # Send converted files to a specific output folder
    python convert_audio.py recordings/ --out converted/

    # Overwrite existing output files
    python convert_audio.py recordings/ --overwrite
"""
...
def collect_inputs(inputs: list[str]) -> list[Path]:
    """Expand files, directories, and glob patterns into a sorted list of WAV paths."""
    paths: list[Path] = []
    for inp in inputs:
        p = Path(inp)
        if p.is_dir():
    ...
    # Deduplicate, preserve order, keep only .wav
    seen: set[Path] = set()
    result: list[Path] = []
    for p in paths:
...
def convert(src: Path, fmt: str, out_dir: Path | None, overwrite: bool) -> bool:
    """Convert one WAV file. Returns True on success."""
    spec = FORMATS[fmt]
