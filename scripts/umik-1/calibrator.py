...
class AudioDeviceCalibrator:
        ...
        if cache_exists and not force_write:
            ...
            try:
                ...
                if len(self._filter_taps) != num_taps:
                    print(f"Warning: Cached filter length ({len(self._filter_taps)}) does not match requested length ({num_taps}). Will redesign.")
                    cache_exists = False
                else:
                ...
                if len(parts) >= 2:
                    try:
                        ...
                        if not data_started:
                            ...
                            print(f"Warning: Found non-numeric data after valid data started (line {line_num}). Stopping parse. Line: '{line}'")
                            break
                elif not data_started:
                    print(f"Skipping potential header/comment line {line_num}: '{line}'")
                    continue
                else:
                    print(f"Warning: Found line with unexpected format after valid data started (line {line_num}). Stopping parse. Line: '{line}'")
                    break

            if not frequencies:
                print(f"❌ FATAL: No valid frequency/gain data pairs (two numeric columns separated by whitespace) found in '{file_path}'.")
                exit(1)
    ...
    def _design_fir_filter(self, freqs: np.ndarray, gains: np.ndarray, num_taps: int) -> np.ndarray:
        ...
        full_gains = np.concatenate(([correction_gains_linear[0]], correction_gains_linear, [correction_gains_linear[-1]]))
                ...
                for line_num, line in enumerate(f):
                    ...
                    if "Sens Factor" in line:
                        try:
                            ...
                            raise ValueError(f"Could not parse 'Sens Factor' line (line {line_num+1}): '{line}'. Expected format like 'Sens Factor =-X.Xd B,...'. Error: {parse_error}")
